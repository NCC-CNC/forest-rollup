...
# Date: July 26th, 2024
#
# Description: prep AAFC LUT to forest, projected from UTM to Canada_Albers_WGS_1984.
#
...
NFIS_PREP <- rast("C:/Data/NAT/Habitat/Forest/Prep/NFIS/_2022/NFIS_ALBERS.tif")  # Change "_2022" need be
...
AAFC_LUTS <- file.path(AAFC_RAW, paste0("LUTS", AAFC_YEAR)) 
...
proj4_string <- as.character(terra::crs(NFIS_PREP), "proj4") # projection string
bbox <- terra::ext(NFIS_PREP) # bounding box
te <- c(bbox[1], bbox[3], bbox[2], bbox[4]) # xmin, ymin, xmax, ymax
ts <- c(terra::ncol(NFIS_PREP), terra::nrow(NFIS_PREP)) # columns/rows
...
for (LU_TIF in LUTS_TIFS) {
  ...
  terra::classify(
    x = rast(LU_TIF), 
    rcl = rclmat,
    filename = file.path(OUT_PREP, AAFC_YEAR,  paste0("UTM_FOREST_",basename(LU_TIF))),
    overwrite = TRUE, 
    datatype = "INT1U"
  )                                        
  
  print("... project to Canada_Albers_WGS_1984 and algin")
  gdalUtilities::gdalwarp(
    srcfile = file.path(OUT_PREP, AAFC_YEAR, paste0("UTM_FOREST_",basename(LU_TIF))),
    dstfile = file.path(OUT_PREP, AAFC_YEAR, paste0("ALBERS_FOREST_",basename(LU_TIF))),
...
treed_tifs <- list.files(
  file.path(OUT_PREP, AAFC_YEAR),  pattern = "^ALBERS.*\\.tif$", full.names = TRUE
)
